...
void blink(void)
{
    if (counter >= blink_geschwindigkeit) {
        PORTD.OUTTGL = PIN7_bm;   /* toggle PD7 */
        counter = 0;
    }
    counter++;
}
...
void zwei_leds_blinken_lassen(void)
{
    ...
    if (counter >= blink_geschwindigkeit) {
        PORTD.OUTTGL = zwei_leds;   /* toggle PD6 and PD7 together */
        counter = 0;
    }
    counter++;
}
...
void Lauflicht(void)
{
    ...
    PORTD.OUTCLR = alle_leds;   /* start with all LEDs off */
 ...
 * A short busy-wait delay makes each step visible.
 */
 ...
 *
 * Example:
 *   binary = 2 (00000010) -> gray = 2 ^ 1 = 3 (00000011)
 *   binary = 3 (00000011) -> gray = 3 ^ 1 = 2 (00000010)
 ...
 * The LED is on while the button is held down, off otherwise.
 *
 * Pull-up resistor: the AVR internal pull-up keeps PC4 HIGH when the
 * button is open. Pressing the button connects PC4 to GND (active-low).
 * Reading HIGH on PC4 therefore means the button is NOT pressed.
 *
 * Note: the condition checks for HIGH (button not pressed = LED on),
 * which matches the active-low wiring used in the lab setup.
 */
void light_by_bouton_a(void)
{
    PORTD.DIRSET  = PIN0_bm;        /* PD0 as output */
    PORTD.OUTCLR  = PIN0_bm;        /* LED initially off */
    PORTC.DIRCLR  = PIN4_bm;        /* PC4 as input */
    PORTC.PIN4CTRL = PORT_PULLUPEN_bm; /* enable internal pull-up on PC4 */

    while (1) {
        if (PORTC.IN & PIN4_bm) {
            PORTD.OUTSET = PIN0_bm;   /* button released: LED on */
        } else {
            PORTD.OUTCLR = PIN0_bm;   /* button pressed: LED off */
        }
    }
}
 ...
 * light_by_bouton_b()
 *
 * Configures all LEDs (PD0–PD7) as outputs and all buttons (PC4–PC7) as
 * inputs with pull-ups. All LEDs turn on when any button is pressed.
 */
void light_by_bouton_b(void)
{
    PORTD.DIRSET = alle_leds;
    PORTD.OUTSET = alle_leds;           /* LEDs initially on */
    ...
    while (1) {
        if (PORTC.IN & alle_taste) {
            PORTD.OUTSET = alle_leds;   /* any button released: LEDs on */
        } else {
            PORTD.OUTCLR = alle_leds;   /* all buttons pressed: LEDs off */
        }
    }
}
 ...
 * The function then waits for the button to be released before accepting
 * the next press, preventing multiple increments from a single press.
 */
void Debounced_Counter(void)
{
    ...
    while (1) {
        if (!(PORTC.IN & PIN4_bm)) {        /* first edge detected */
            _delay_ms(30);                   /* wait for bounce to settle */

            if (!(PORTC.IN & PIN4_bm)) {    /* confirm button is still down */
                PORTD.OUT = zaeler;
                zaeler++;
 ...
 *
 * Button assignments (defined in leds.h):
 ...
 *
 * Edge detection: the result is updated only on a new press,
 * not while the button is held. This is achieved by comparing
 * the current button state with the previous state.
...
void Binary_calculator(void)
{
    ...
    while (1) {
        ...
        /* act only on a new press, not on a held button */
 ...
 * button_by_light()
 *
 * A reaction game: a random LED lights up and the player must press the
 * matching button to turn it off. A new random LED then lights up.
 *
 * LED-to-button mapping (both on pins PC4–PC7 / PD4–PD7 range):
 *   led_index 0 -> PIN4_bm
 *   led_index 1 -> PIN5_bm
 *   led_index 2 -> PIN6_bm
 *   led_index 3 -> PIN7_bm
 *
 * rand() % 4 produces a value in [0, 3].
 * Shifting PIN4_bm left by led_index selects the corresponding pin.
 */
void button_by_light(void)
{
    ...
    while (1) {
        ...
        PORTD.OUT = led_choise;   /* light the selected LED */

        /* wait until the matching button is pressed */
        while (1) {
            uint8_t gedrueckte_Button = (PORTC.IN & alle_taste);

            if (gedrueckte_Button == led_choise) {
                PORTD.OUTCLR = led_choise;   /* turn off the LED */
                break;
            }
            _delay_ms(30);
        }
